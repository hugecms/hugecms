...
class ContentEntity implements \JsonSerializable
{
    ...
    public const string getTitle = 'title'; // 内容标题

    public const string getSlug = 'slug'; // URL别名（全局唯一，由应用层从标题生成，避免空串重复占位）

    public const string getAuthorId = 'author_id'; // 发布者用户ID

    public const string getStatus = 'status'; // 状态：draft草稿/pending待发布(定时)/published已发布/archived已归档/trash回收站

    public const string getVisibility = 'visibility'; // 可见性：public公开/password密码保护/private私密（仅登录可见），与 status/audit_status 独立（见 docs/development-conventions.md 状态机）

    public const string getPassword = 'password'; // 密码保护口令（visibility=password 时使用，哈希存储）

    public const string getViews = 'views'; // 浏览量计数

    public const string getCommentCount = 'comment_count'; // 评论数（审核通过的冗余计数，避免列表页逐条COUNT）

    public const string getSort = 'sort'; // 手动排序权重（数值越大越靠前）

    public const string getIsTop = 'is_top'; // 是否置顶：1置顶（列表排序优先；置顶属列表行为而非内容属性，故为全模型公共列）

    public const string getPublishedAt = 'published_at'; // 计划/实际发布时间

    public const string getAuditStatus = 'audit_status'; // 审核状态：pending待审核/approved通过/rejected驳回

    public const string getAuditRemark = 'audit_remark'; // 审核备注（驳回原因）

    public const string getAuditorId = 'auditor_id'; // 审核人ID

    public const string getAuditedAt = 'audited_at'; // 审核时间
    ...
    #[OA\Property(property: 'password', description: '密码保护口令（visibility=password 时使用，哈希存储）', type: 'string')]
    ...
    /**
     * 获取密码保护口令（visibility=password 时使用，哈希存储）
     */
    ...
    /**
     * 设置密码保护口令（visibility=password 时使用，哈希存储）
     */
