...
#[OA\Schema(schema: 'TermEntity')]
class TermEntity implements \JsonSerializable
{
    use HasSerializableAttributes;

    public const string getId = 'id'; // ID

    public const string getTaxonomyId = 'taxonomy_id'; // 所属分类法ID

    public const string getName = 'name'; // 分类项名称（如：科技、体育）

    public const string getSlug = 'slug'; // 分类项别名（URL友好）

    public const string getParentId = 'parent_id'; // 父级ID（0代表顶级）

    public const string getDescription = 'description'; // 分类项描述

    public const string getSort = 'sort'; // 排序权重

    public const string getContentCount = 'content_count'; // 该分类下已发布内容数（冗余计数，对标 wp_term_taxonomy.count；仅统计 status=published 且 audit_status=approved；增删关联或发布状态变更时事务维护，见 docs/development-conventions.md）
