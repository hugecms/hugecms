...
#[OA\Schema(schema: 'ContentPushQueueEntity')]
class ContentPushQueueEntity implements \JsonSerializable
{
    use HasSerializableAttributes;

    public const string getId = 'id'; // ID

    public const string getContentId = 'content_id'; // 被推送的内容ID

    public const string getPushType = 'push_type'; // 推送目标：wechat_mp/miniprogram/weibo/baidu_zhanzhang/rss

    public const string getPushData = 'push_data'; // 推送数据的最终形态（预处理后JSON）

    public const string getStatus = 'status'; // 状态：pending/processing/success/failed（执行与重试走 Laravel 队列）

    public const string getRetryCount = 'retry_count'; // 已重试次数

    public const string getMaxRetries = 'max_retries'; // 最大重试次数

    public const string getErrorMessage = 'error_message'; // 失败时的错误信息

    public const string getFinishedAt = 'finished_at'; // 完成时间
