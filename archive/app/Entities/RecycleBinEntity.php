...
#[OA\Schema(schema: 'RecycleBinEntity')]
class RecycleBinEntity implements \JsonSerializable
{
    use HasSerializableAttributes;

    public const string getId = 'id'; // ID

    public const string getDeletedBy = 'deleted_by'; // 删除人用户ID

    public const string getTargetType = 'target_type'; // 原对象类型：content/term/attachment/user/form_submission/comment

    public const string getTargetId = 'target_id'; // 原对象ID

    public const string getOriginalData = 'original_data'; // 删除前的全量数据快照（JSON）

    public const string getRestoreData = 'restore_data'; // 恢复时所需的数据映射（如恢复时需新建ID）

    public const string getRetentionDays = 'retention_days'; // 保留天数（超时由 Scheduler 物理清除）

    public const string getCreatedAt = 'created_at'; // 删除时间

    public const string getExpireAt = 'expire_at'; // 过期时间（虚拟生成列）
