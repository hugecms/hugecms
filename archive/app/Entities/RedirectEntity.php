...
#[OA\Schema(schema: 'RedirectEntity')]
class RedirectEntity implements \JsonSerializable
{
    use HasSerializableAttributes;

    public const string getId = 'id'; // ID

    public const string getSourcePath = 'source_path'; // 来源路径（站内相对路径，以 / 开头）

    public const string getTargetPath = 'target_path'; // 目标路径（相对路径或完整URL）

    public const string getStatusCode = 'status_code'; // HTTP状态码：301永久重定向，302临时重定向

    public const string getHits = 'hits'; // 命中次数（冗余计数，事务内维护）

    public const string getLastHitAt = 'last_hit_at'; // 最后命中时间

    public const string getRemark = 'remark'; // 备注（如：slug 改版、栏目迁移）
