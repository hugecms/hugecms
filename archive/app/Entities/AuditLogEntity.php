...
#[OA\Schema(schema: 'AuditLogEntity')]
class AuditLogEntity implements \JsonSerializable
{
    use HasSerializableAttributes;

    public const string getId = 'id'; // ID

    public const string getUserId = 'user_id'; // 操作用户ID

    public const string getUserName = 'user_name'; // 操作用户名（冗余，防用户被删后无记录）

    public const string getClientIp = 'client_ip'; // 客户端IP（支持IPv6）

    public const string getUserAgent = 'user_agent'; // 客户端UA信息

    public const string getRequestId = 'request_id'; // 请求追踪ID（关联一次请求的所有日志）

    public const string getEventType = 'event_type'; // 事件类型：LOGIN/LOGOUT/CREATE/UPDATE/DELETE/PUBLISH/EXPORT

    public const string getTargetType = 'target_type'; // 目标类型：content/term/user/attachment/config/comment/form_submission

    public const string getTargetId = 'target_id'; // 目标ID（可能是数字或UUID）

    public const string getTargetName = 'target_name'; // 目标名称（冗余，便于展示）

    public const string getOldValue = 'old_value'; // 修改前的数据快照（JSON）

    public const string getNewValue = 'new_value'; // 修改后的数据快照（JSON）

    public const string getOperationResult = 'operation_result'; // 操作结果：0失败，1成功

    public const string getErrorMessage = 'error_message'; // 失败时的错误信息

    public const string getCreatedAt = 'created_at'; // 创建时间（毫秒精度，只增不改）
